...
// Fire a learned code by writing it to `ir_send`.
//
...
//
// Callers pass the plain code; the wrapping lives only here. `properties` is a
// JSON *string* whose `ir_send` is itself a JSON string — two layers on purpose.
export async function sendIr(deviceId, code) {
  const command = JSON.stringify({
    control: 'send_ir', head: '', key1: '1' + code, type: 0, delay: 300,
  });
  return tuyaRequest('POST',
    '/v2.0/cloud/thing/' + encodeURIComponent(deviceId) + '/shadow/properties/issue',
    { properties: JSON.stringify({ ir_send: command }) });
}
