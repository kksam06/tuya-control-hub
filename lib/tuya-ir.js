...
// Fire a learned code by writing it to `ir_send`. Tuya wants `properties` as a
// JSON *string*, the exact form that made the fan react in API Explorer.
export async function sendIr(deviceId, code) {
  return tuyaRequest('POST',
    ...
    { properties: JSON.stringify({ ir_send: code }) });
}
