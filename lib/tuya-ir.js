...
//   TUYA_DEVICE_ID_AIR_BOX        optional: the air-quality box read by api/air-status.js
//
...
function readConfig() {
  ...
  // Kept apart from deviceIds: the air box can only be read, never sent IR.
  const airBoxIds = (process.env.TUYA_DEVICE_ID_AIR_BOX || '')
    .split(',').map(s => s.trim()).filter(Boolean);
  return { accessId, secret, deviceIds, airBoxIds };
}
...
let tokenInFlight = null;   // one token request at a time, however many callers wait on it
async function getToken(force) {
  ...
  // Requests made together (ir-status reads the box and its online flag at
  // once) share the one token fetch instead of each starting their own.
  if (!tokenInFlight) {
    tokenInFlight = (async () => {
...
// Is the device connected to Tuya's cloud right now? The shadow below keeps
// serving the last values of a box that has dropped off WiFi, so a successful
// read of it says nothing about this. true / false, or null when Tuya's reply
// carries no answer.
export async function getOnline(deviceId) {
  const result = await tuyaRequest('GET', '/v2.0/cloud/thing/' + encodeURIComponent(deviceId));
  return typeof result?.is_online === 'boolean' ? result.is_online : null;
}
...
// What each data point is — name, unit and `scale` (the reported number is the
// real value × 10^scale). It does not change, so each warm instance asks once.
const modelCache = new Map();   // deviceId → parsed model
export async function getThingModel(deviceId) {
  if (modelCache.has(deviceId)) return modelCache.get(deviceId);
  const result = await tuyaRequest('GET',
    '/v2.0/cloud/thing/' + encodeURIComponent(deviceId) + '/model');
  // `model` arrives as a JSON string inside the JSON reply.
  const model = typeof result?.model === 'string' ? JSON.parse(result.model) : (result?.model || null);
  if (model) modelCache.set(deviceId, model);
  return model;
}
...
// `preToken` — the token of the previous press — makes this a refinement: Tuya
// keeps only the code sets that match this press AND the earlier ones.
export async function startMatching(deviceId, categoryId, code, preToken) {
  const body = { category_id: categoryId, code };
  if (preToken) body.pre_token = preToken;
  return tuyaRequest('POST', ir(deviceId) + '/matching-remotes/token', body);
}
...
// The air box to read. As with pickDevice, only one from its own list. When no
// air box is configured the caller gets a 404, which the web reads as "none".
export function pickAirBox(requested) {
  if (!cfg) cfg = readConfig();
  if (!cfg.airBoxIds.length) {
    const e = new Error('No air box configured (TUYA_DEVICE_ID_AIR_BOX)');
    e.status = 404;
    throw e;
  }
  const id = String(requested || '').trim();
  if (!id) return cfg.airBoxIds[0];
  return cfg.airBoxIds.includes(id) ? id : null;
}
